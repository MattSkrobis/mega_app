...
describe Editor::CommentsController do
  ...
  describe '#show' do
    let(:call_request) { get :show, user_id: user.id, id: comment.id }
    ...
    context 'after request' do
      ...
      it { expects(assigns(:comment)).to eq comment }
    end
  end

  describe '#delete' do
    let!(:comment) { create(:comment, user: user, book: book) }
    let(:call_request) { delete :destroy, book_id: book.id, user_id: user.id, id: comment.id }
  ...
  describe '#update' do
    ...
    context "valid request" do
      ...
      it { expects { call_request }.to change { comment.reload.rating }.from(3).to(6) }
    ...
    context "invalid request" do
      ...
      it { expects { call_request }.not_to change { comment.reload.rating }}
